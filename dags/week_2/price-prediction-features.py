...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["generation coal all"] = (
        df["generation fossil hard coal"] + df["generation fossil brown coal/lignite"]
    )
    return df
...
def prepare_model_inputs(df_final: pd.DataFrame):
    ...
    write_bucket.blob(TRAINING_DATA_PATH).upload_from_string(
        pd.DataFrame(dataset_norm).to_csv()
    )
...
with DAG(
    "energy_price_prediction_features",
